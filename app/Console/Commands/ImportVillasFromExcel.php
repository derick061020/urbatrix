...
class ImportVillasFromExcel extends Command
{
    ...
    /** Hoja «Precios» del Excel: [código de villa => precio y vista]. */
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            // Si el Excel trae la hoja «Precios», manda ella: el precio real de
            // cada lote y su vista. Si no, se cae a los estimados de abajo.
            $this->priceList = VillaPriceList::exists($file) ? VillaPriceList::read($file) : [];
        ...
        $this->line($this->priceList !== []
            ? 'Precios: hoja «' . VillaPriceList::SHEET . '» (' . count($this->priceList) . ' villas con precio real)'
            : 'Precios: estimados por tipología (el Excel no trae hoja «' . VillaPriceList::SHEET . '»)');
    ...
    private function mapVilla(array $row, array $typ, ?int $projectId): array
    {
        ...
        return [
            ...
            'price'      => $this->price($typ, $total, $unitCode),
            ...
            'outlook'   => $this->outlook($typ, $unitCode),
    ...
    private function price(array $typ, float $totalM2, string $unitCode = ''): float
    {
        // --price-per-m2 es un override explícito: gana sobre todo lo demás.
        if ($perM2 = (float) $this->option('price-per-m2')) {
            return round($perM2 * $totalM2, 2);
        }

        if ($real = $this->priceList[strtoupper($unitCode)]['price'] ?? null) {
            return (float) $real;
        }
    ...
    /**
     * Vista de la villa. La hoja «Precios» la da por unidad (Mar / Montaña /
     * Sin vista) y mapeada al catálogo de opciones, así que el filtro del home
     * funciona; la hoja «Tipologias» sólo tiene un texto libre por modelo.
     */
    private function outlook(array $typ, string $unitCode): ?string
    {
        $row = $this->priceList[strtoupper($unitCode)] ?? null;
        if ($row !== null) {
            return VillaPriceList::mapView($row['view']);
        }

        return $typ['outlook'] ?: null;
    }
    ...
    /**
     * Filas de una hoja como [clave lógica => valor]. La lectura del .xlsx
     * vive en {@see XlsxSheet}, compartida con villas:prices.
     *
    ...
    {
        return XlsxSheet::rows($file, $sheetName, $patterns);
    }
