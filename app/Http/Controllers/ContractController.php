...
3. A final payment upon delivery for the sum of " . $this->numberToEnglish(round($totalPrice * 0.09)) . " UNITED STATES DOLLARS WITH 00/100 (US\$" . number_format($totalPrice * 0.09, 2, '.', ',') . "), equivalent to the remaining 9% of the property price.";
    ...
    public function generatePaymentPlan(Reservation $reservation)
    {
        ...
        try {
            ...
            // Generate installment dates and data (only if there are installments)
            $cuotas = [];
            if ($breakdown['cantidad_cuotas'] > 0) {
                ...
                // Clone rows for installments table (with error handling)
                try {
                    // Buscar el marcador que existe en el template
                    $templateProcessor->cloneRow('cuota_numero#1', $breakdown['cantidad_cuotas']);

                    // Set values for each installment row
                    ...
                    // If cloneRow fails, continue without table
            ...
            // Create or update document record
            $document = DocumentService::getDocumentByType($reservation, 'payment_plan');
            if ($document) {
                $document->update([
                    'file_path' => $filePath,
                    'filename' => $fileName,
