    ...
    public function admin()
    {
        ...
        foreach ($msgQuery->latest()->limit(10)->get() as $m) {
            $items->push([
                ...
                'icon'       => 'pi-comment',
                'color'      => 'blue',
                'title'      => __('Nuevo mensaje de :name', ['name' => $m->sender->name ?? __('cliente')]),
        ...
        foreach ($payQuery->latest()->limit(10)->get() as $p) {
            $items->push([
                ...
                'icon'       => 'pi-credit-card',
                'color'      => 'amber',
                'title'      => __('Pago pendiente de aprobación'),
                'body'       => 'RD$ '.number_format((float) $p->amount, 2).' · '.$p->display_label,
        ...
        if (! $isBroker) {
            ...
            if (Schema::hasColumn('users', 'verification_status')) {
                foreach (User::where('verification_status', 'pending')->latest()->limit(10)->get() as $usr) {
                    $items->push([
                        ...
                        'icon'       => 'pi-id-card',
                        'color'      => 'amber',
                        'title'      => __('Verificación KYC pendiente'),
            ...
            foreach ($exportRequests as $ex) {
                ...
                $items->push([
                    ...
                    'icon'       => 'pi-key',
                    'color'      => 'amber',
                    'title'      => __('Código de exportación').' · '.$ex->code,
                    'body'       => __(':user solicitó exportar :resource. Compártele este código.', ['user' => $ex->requester->name ?? __('Usuario'), 'resource' => $resourceLabel]),
            ...
            foreach (Task::whereIn('status', ['pendiente', 'vencida'])->latest()->limit(8)->get() as $t) {
                $items->push([
                    ...
                    'title'      => $t->status === 'vencida' ? __('Tarea vencida') : __('Tarea pendiente'),
                    'body'       => $t->title ?: __('Tarea #:id', ['id' => $t->id]),
    ...
    public function client()
    {
        ...
        foreach ($unread as $m) {
            $items->push([
                ...
                'icon'       => 'pi-comment',
                'color'      => 'blue',
                'title'      => __('Mensaje de :name', ['name' => $m->sender->name ?? 'Admin']),
                'body'       => \Illuminate\Support\Str::limit((string) $m->body, 90),
                'created_at' => $m->created_at,
        ...
        if ($nextPay) {
            $items->push([
                ...
                'icon'       => 'pi-credit-card',
                'color'      => 'amber',
                'title'      => __('Próximo pago pendiente'),
                'body'       => 'RD$ '.number_format((float) $nextPay->amount, 2).' · '.$nextPay->display_label,
        ...
        if ($reservationIds->isNotEmpty()) {
            ...
            foreach ($budgetReady as $r) {
                $items->push([
                    ...
                    'icon'       => 'pi-file-edit',
                    'color'      => 'green',
                    'title'      => __('Presupuesto disponible'),
                    'body'       => __('Revisa el presupuesto de tu reserva :code', ['code' => $r->reservation_code ? '#'.$r->reservation_code : '']),
        ...
        $items = $items
            ...
            ->map(function ($i) use ($lastSeenAt) {
                ...
                return [
                    ...
                    'title'   => $i['title'],
                    'body'    => $i['body'],
                    'when'    => $when ? $when->locale(app()->getLocale())->diffForHumans() : '',
                    'url'     => $i['url'],
