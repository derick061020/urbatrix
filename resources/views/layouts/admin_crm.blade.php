...
<head>
    ...
    <title>@yield('title', __('CRM Landmass - Bahía Mar'))</title>
    ...
    <style>
      ...
      .crm-tab.active { background:#074540; color:#fff; }
      .crm-tab-line {
      ...
      .crm-btn-primary { background:#074540; color:#fff; border:1px solid #074540; }
      .crm-btn-primary:hover { background:#053330; border-color:#053330; }
...
<div class="min-h-screen flex p-2 sm:p-3 gap-2 sm:gap-3">
    ...
    <aside id="crm-sidebar" class="w-[220px] shrink-0 flex flex-col h-[calc(100vh-24px)] sticky top-3">
        ...
        <a href="/" class="rounded-xl bg-white border border-ink-200 px-3 py-2.5 flex items-center gap-2.5 hover:bg-ink-50 transition-colors cursor-pointer no-underline">
            <span class="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 shadow-xs" style="background:#ffffff;border:1px solid rgba(0,0,0,0.06)">
        ...
        <div class="mt-2 rounded-xl bg-white border border-ink-200">
            <div class="flex items-center gap-2.5 px-3 py-2.5">
                <button type="button" class="crm-avatar shrink-0 border-0 p-0 cursor-pointer" style="background:#074540; {{ Auth::user()->avatar ? 'background-image:url('.asset('storage/'.Auth::user()->avatar).');background-size:cover;background-position:center;color:transparent;' : '' }}" title="{{ __('Editar') }}" onclick="openSettingsModal()">
    ...
    <div class="flex-1 min-w-0 flex flex-col rounded-2xl bg-white shadow-panel border border-ink-200 overflow-hidden">
        ...
        <header id="crm-topbar" class="h-[72px] bg-white border-b border-ink-100 flex items-center px-7 gap-3 shrink-0">
            ...
            <div class="flex-1 min-w-0">
                <h1 class="font-display text-[18px] sm:text-[22px] font-semibold text-ink-950 leading-tight tracking-tight truncate">@yield('page_title', __('Dashboard'))</h1>
                <p class="text-[11px] sm:text-[12px] text-ink-500 leading-tight mt-0.5 truncate">@yield('page_breadcrumb', __('Vista general'))</p>
            </div>
