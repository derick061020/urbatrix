...
<x-mail-shell
    ...
    <tr>
      <td style="background-color:#ffffff;padding:32px 36px 24px 36px;">
        ...
        <p style="margin:0;font-size:13px;color:#6a6a64;line-height:1.65;">{{ __('Confirmamos la recepción de tu pago del') }} <strong style="color:#1a1a18;">{{ $d['fecha_pago'] }}</strong> {{ __('para tu') }} <strong style="color:#1a1a18;">Unidad {{ $d['unidad'] }}</strong> en <strong style="color:#1a1a18;">{{ $d['proyecto'] }}</strong>{{ __('. A continuación encontrarás el resumen de la operación.') }}</p>
      </td>
    </tr>
    ...
    <tr>
      <td style="background-color:#ffffff;padding:0 36px 10px 36px;">
        <p style="margin:0;font-size:9px;font-weight:600;color:#8a8a84;letter-spacing:0.22em;text-transform:uppercase;">{{ __('Resumen del pago') }}</p>
      </td>
    </tr>
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border-top:1px solid #e8e7e3;">
          <tr>
            <td style="width:50%;padding:13px 16px 13px 0;border-bottom:1px solid #e8e7e3;vertical-align:top;">
              <p style="margin:0 0 4px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.14em;text-transform:uppercase;">{{ __('Unidad') }}</p>
            ...
            <td style="width:50%;padding:13px 0 13px 16px;border-bottom:1px solid #e8e7e3;vertical-align:top;border-left:1px solid #e8e7e3;">
              <p style="margin:0 0 4px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.14em;text-transform:uppercase;">{{ __('Concepto') }}</p>
              <p style="margin:0;font-size:15px;font-weight:300;color:#1a1a18;">{{ $d['concepto_pago'] }}</p>
            </td>
          </tr>
          <tr>
            <td style="width:50%;padding:13px 16px 13px 0;border-bottom:1px solid #e8e7e3;vertical-align:top;">
              <p style="margin:0 0 4px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.14em;text-transform:uppercase;">{{ __('Método') }}</p>
              <p style="margin:0;font-size:12px;font-weight:500;color:#1a1a18;">{{ $d['metodo_pago'] }}</p>
            </td>
            <td style="width:50%;padding:13px 0 13px 16px;border-bottom:1px solid #e8e7e3;vertical-align:top;border-left:1px solid #e8e7e3;">
              <p style="margin:0 0 4px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.14em;text-transform:uppercase;">{{ __('Referencia') }}</p>
          ...
          <tr>
            <td style="width:50%;padding:12px 16px 12px 0;border-bottom:1px solid #e8e7e3;vertical-align:top;">
              <p style="margin:0 0 3px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.12em;text-transform:uppercase;">{{ __('Total pagado a la fecha') }}</p>
            ...
            <td style="width:50%;padding:12px 0 12px 16px;border-bottom:1px solid #e8e7e3;vertical-align:top;border-left:1px solid #e8e7e3;">
              <p style="margin:0 0 3px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.12em;text-transform:uppercase;">{{ __('Saldo pendiente') }}</p>
    ...
    <tr>
      <td style="background-color:#ffffff;padding:24px 36px 8px 36px;">
        <p style="margin:0 0 12px 0;font-size:12px;color:#6a6a64;line-height:1.65;">{{ __('El comprobante completo en PDF está disponible en tu portal. Guárdalo como respaldo oficial de tu pago.') }}</p>
        <table role="presentation" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td style="background-color:#0b1c0a;">
              <a href="{{ $d['link_comprobante'] }}" style="display:inline-block;padding:11px 24px;font-size:11px;font-weight:600;color:#F1EDE3;text-decoration:none;letter-spacing:0.1em;text-transform:uppercase;">{{ __('Descargar comprobante PDF') }}</a>
            </td>
            <td style="width:16px;"></td>
            <td style="vertical-align:middle;">
              <a href="{{ $d['link_portal'] }}" style="font-size:11px;color:#8a8a84;text-decoration:underline;">{{ __('Ver mi portal') }}</a>
            </td>
          </tr>
        </table>
      </td>
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
          <tr>
            <td style="padding:0 0 0 20px;border-left:2px solid #B8962E;">
              <p style="margin:0 0 2px 0;font-size:12px;color:#6a6a64;line-height:1.6;">{{ __('Para cualquier consulta sobre este pago escríbenos a') }} <strong style="color:#1a1a18;font-weight:600;">{{ $c['support_email'] }}</strong>@if($d['nombre_asesor'] !== '—') o contacta a tu asesor <strong style="color:#1a1a18;font-weight:600;">{{ $d['nombre_asesor'] }}</strong>@endif.</p>
              <p style="margin:4px 0 0 0;font-size:12px;color:#6a6a64;">{{ __('Gracias por tu confianza.') }}</p>
            </td>
          </tr>
        </table>
      </td>
