...
<x-mail-shell
    ...
    <tr>
      <td style="background-color:#ffffff;padding:32px 36px 18px 36px;">
        ...
        <p style="margin:0;font-size:13px;color:#6a6a64;line-height:1.65;">{{ __('Nos complace compartirte las últimas novedades del avance de construcción de tu inversión en') }} <strong style="color:#1a1a18;">{{ $proyecto }}</strong>{{ __('. Estamos trabajando con el mayor compromiso para entregarte una propiedad de la más alta calidad.') }}</p>
      </td>
    </tr>
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border-top:1px solid #e8e7e3;">
          <tr>
            <td colspan="2" style="background-color:#0b1c0a;padding:20px 18px;">
              <p style="margin:0 0 4px 0;font-size:9px;font-weight:500;color:rgba(184,150,46,0.7);letter-spacing:0.14em;text-transform:uppercase;">{{ __('Progreso actual de obra') }}</p>
              <p style="margin:0;font-size:30px;font-weight:600;color:#F1EDE3;letter-spacing:-0.01em;">{{ $report->overall_progress }}<span style="font-size:18px;">%</span> <span style="font-size:11px;font-weight:400;color:rgba(241,237,227,0.5);">{{ __('completado') }}</span></p>
              <!-- barra -->
          ...
          <tr>
            <td style="width:50%;padding:14px 16px 14px 0;border-bottom:1px solid #e8e7e3;border-top:1px solid #e8e7e3;vertical-align:top;">
              <p style="margin:0 0 3px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.12em;text-transform:uppercase;">{{ __('Período') }}</p>
            ...
            <td style="width:50%;padding:14px 0 14px 16px;border-bottom:1px solid #e8e7e3;border-top:1px solid #e8e7e3;vertical-align:top;border-left:1px solid #e8e7e3;">
              <p style="margin:0 0 3px 0;font-size:9px;font-weight:500;color:#8a8a84;letter-spacing:0.12em;text-transform:uppercase;">{{ __('Entrega estimada') }}</p>
    ...
    <tr>
      <td style="background-color:#ffffff;padding:24px 36px 8px 36px;">
        <p style="margin:0 0 12px 0;font-size:12px;color:#6a6a64;line-height:1.65;">{{ __('Accede a la galería fotográfica completa y al detalle de los hitos de construcción en tu portal.') }}</p>
        <table role="presentation" cellspacing="0" cellpadding="0" border="0">
          <tr><td style="background-color:#0b1c0a;">
            <a href="{{ $linkPortal }}" style="display:inline-block;padding:11px 24px;font-size:11px;font-weight:600;color:#F1EDE3;text-decoration:none;letter-spacing:0.1em;text-transform:uppercase;">{{ __('Ver galería y reporte') }}</a>
          </td></tr>
        </table>
      </td>
    </tr>
