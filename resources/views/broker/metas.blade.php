...
<div class="p-4 sm:p-6 lg:p-7 space-y-5">
    ...
    <p class="text-[12px] text-ink-500">{{ __('Tu progreso, tu nivel y el ranking del equipo. Todo sobre ventas reales.') }}</p>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {{-- Meta del trimestre --}}
        <div class="brk-card p-5">
            <div class="flex items-center justify-between mb-3">
                <span class="text-[14px] font-bold text-ink-950">{{ __('Meta del trimestre') }}</span>
                <span class="brk-pill bg-brand-tint text-brand-dark">{{ __('Trimestre') }}</span>
            </div>
        ...
        {{-- Tu nivel --}}
        <div class="brk-card p-5">
            <div class="text-[14px] font-bold text-ink-950 mb-3">{{ __('Tu nivel') }}</div>
            <div class="flex items-center justify-between text-[12px] mb-1.5"><span class="font-semibold text-ink-900">{{ __('Plata → Oro') }}</span><span class="text-ink-500">{{ $goalProgress }} / {{ $levelTarget }} ventas/trim</span></div>
            ...
            <p class="text-[12px] text-ink-500 mt-2.5">{{ __('Oro desbloquea') }} <b>{{ __('+1% de comisión') }}</b> {{ __('y acceso anticipado a nuevo inventario.') }}</p>
        </div>
    </div>
    ...
    <div class="brk-card p-4">
        <div class="flex items-center gap-3 bg-[#FBF3DF] border border-[#EFE0BD] rounded-xl p-3.5">
            ...
            <div>
                <div class="text-[13px] font-bold text-[#6b5410]">{{ __('Campaña activa · +2% de comisión') }}</div>
                <div class="text-[11.5px] text-[#8a6f25]">{{ __('En reservas cerradas durante el período de campaña. Acumulable con tu % base.') }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="brk-card overflow-hidden">
        <div class="px-5 py-3 border-b border-ink-100 flex items-center justify-between">
            <span class="text-[14px] font-bold text-ink-950">{{ __('Ranking · este trimestre') }}</span>
            <span class="text-[11px] text-ink-400">{{ __('por cierres') }}</span>
        </div>
        <div class="p-3 sm:p-4">
            ...
            @empty
                <div class="text-[12px] text-ink-400 text-center py-6">{{ __('Aún no hay datos de ranking.') }}</div>
            @endforelse
            <p class="text-[11px] text-ink-400 mt-3">{{ __('El ranking premia cierres reales — no solo actividad — para no incentivar ventas forzadas.') }}</p>
        </div>
    </div>

    {{-- Logros --}}
    <div class="brk-card p-5">
        <div class="text-[14px] font-bold text-ink-950 mb-3">{{ __('Tus logros') }}</div>
