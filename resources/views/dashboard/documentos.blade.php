...
<div class="p-4 sm:p-6 lg:p-7 space-y-5">
    ...
    {{-- Header --}}
    <div>
        <h2 class="font-display text-[20px] font-semibold text-ink-950 leading-tight">Tus documentos</h2>
        <p class="text-[13px] text-ink-500 mt-1">Acá ves los documentos firmados o aprobados, listos para descargar. Los pendientes de revisión están en <a href="{{ route('dashboard.acuerdos') }}" class="text-brand font-semibold hover:underline">Acuerdos</a>.</p>
    </div>
    ...
    <div class="cli-card overflow-hidden">
        <div class="px-5 py-3 flex items-center gap-3 bg-ok-soft/60 border-b border-ok/20">
            ...
            <div class="flex-1">
                <div class="text-[14px] font-bold text-ink-950">Contratos y planes firmados</div>
                <div class="text-[11px] text-ink-500">Documentos finalizados de tu expediente</div>
            </div>
            <span class="text-[11px] text-ink-500">{{ $signedDocs->count() }} archivo{{ $signedDocs->count() === 1 ? '' : 's' }}</span>
        </div>

        @if($signedDocs->isEmpty())
            <div class="px-5 py-8 text-center text-[12px] text-ink-500">
                Todavía no hay documentos firmados o aprobados. Cuando firmes en <a href="{{ route('dashboard.acuerdos') }}" class="text-brand font-semibold hover:underline">Acuerdos</a>, aparecerán acá.
            </div>
        @else
            <table class="w-full">
                <thead class="bg-white">
                    <tr>
                        ...
                        <th class="text-left px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-ink-500">Firmado</th>
                        <th class="text-right px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-ink-500">Acciones</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-ink-50">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-[13px] font-semibold text-ink-950">{{ $d->title ?? ($typeLabel[$d->document_type] ?? 'Documento') }}</div>
                            ...
                            <td class="px-3 py-4 text-[12px] text-ink-700">{{ $when ? \Carbon\Carbon::parse($when)->locale('es')->isoFormat('D MMM YYYY') : '—' }}</td>
                            <td class="px-3 py-4 text-right">
                                <div class="flex items-center gap-2 justify-end">
                                    @if($d->file_path)
                                        <button type="button" onclick="openDocumentPreview({{ json_encode(['url' => route('documents.preview', $d->id), 'title' => $d->title ?? ($typeLabel[$d->document_type] ?? 'Documento'), 'filename' => $d->filename ?? basename((string) $d->file_path)]) }})" class="cli-btn cli-btn-ghost text-[11px] py-1 px-3"><i class="pi pi-eye text-[10px]"></i> Ver</button>
                                        <a href="{{ route('documents.download', $d->id) }}" class="cli-btn cli-btn-primary text-[11px] py-1 px-3"><i class="pi pi-download text-[10px]"></i> Descargar</a>
                                    @else
                                        <span class="text-[11px] text-ink-400">Sin archivo</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
    ...
    <div class="cli-card overflow-hidden">
        <div class="px-5 py-3 flex items-center gap-3 bg-ink-50/60 border-b border-ink-100">
            ...
            <div class="flex-1">
                ...
                <div class="text-[11px] text-ink-500">Tus archivos KYC entregados al expediente</div>
            </div>
            <span class="text-[11px] text-ink-500">{{ $kycDocs->count() }} archivo{{ $kycDocs->count() === 1 ? '' : 's' }}</span>
        </div>

        @if($kycDocs->isEmpty())
            <div class="px-5 py-8 text-center text-[12px] text-ink-500">No subiste documentos de identidad todavía.</div>
        @else
            <table class="w-full">
                <thead class="bg-white">
                    <tr>
                ...
                <tbody class="divide-y divide-ink-100">
                    @foreach($kycDocs as $d)
                        ...
                        <tr class="hover:bg-ink-50">
                            ...
                            <td class="px-3 py-4"><span class="cli-pill bg-{{ $st[1] }}-soft text-{{ $st[1] }}">{{ $st[0] }}</span></td>
                            <td class="px-3 py-4 text-[12px] text-ink-700">{{ optional($d->created_at)->locale('es')->isoFormat('D MMM YYYY') }}</td>
                            <td class="px-3 py-4 text-right">
                                <div class="flex items-center gap-2 justify-end">
                                    @if($d->file_path)
                                        <button type="button" onclick="openDocumentPreview({{ json_encode(['url' => route('documents.preview', $d->id), 'title' => $d->title ?? ($typeLabel[$d->document_type] ?? 'Documento'), 'filename' => $d->filename ?? basename((string) $d->file_path)]) }})" class="cli-btn cli-btn-ghost text-[11px] py-1 px-3"><i class="pi pi-eye text-[10px]"></i> Ver</button>
    ...
    <div class="w-[min(1040px,95vw)] bg-white rounded-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-ink-100 flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
                <div id="document-preview-title" class="text-[15px] font-bold text-ink-900 truncate">Vista previa</div>
        ...
        <div class="bg-ink-50 p-4">
            <div class="bg-white border border-ink-100 rounded-xl overflow-hidden min-h-[70vh] flex items-center justify-center">
                <iframe id="document-preview-frame" class="hidden w-full h-[70vh] bg-white" title="Vista previa del documento"></iframe>
                <img id="document-preview-image" class="hidden max-h-[70vh] max-w-full object-contain" alt="Vista previa del documento">
                <div id="document-preview-empty" class="hidden text-center px-6 py-12">
                    ...
                    <div class="text-[14px] font-semibold text-ink-900">Vista previa no disponible</div>
                    <div class="text-[12px] text-ink-500 mt-1">Este tipo de archivo no se puede mostrar directamente en el navegador.</div>
                </div>
            </div>
        </div>

        <div class="px-6 py-4 border-t border-ink-100 flex items-center justify-end bg-white">
            <button type="button" onclick="this.closest('dialog').close()" class="cli-btn cli-btn-ghost">Cerrar</button>
        </div>
    </div>
</dialog>
