...
<div class="cli-card overflow-hidden">
    <div class="px-5 py-3 flex items-center gap-3 bg-ok-soft/40 border-b border-ok/20">
        ...
        @if($accepted)
            <span class="cli-pill bg-ok-soft text-ok">{{ __('Aceptado') }}</span>
        @elseif($needsAdminAction)
            <span class="cli-pill bg-info-soft text-info">{{ __('Pendiente de respuesta del asesor') }}</span>
        @elseif($isSent)
            <span class="cli-pill bg-warn-soft text-warn-dark">{{ __('Pendiente de tu respuesta') }}</span>
        @endif
    </div>
    ...
    <div class="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div class="border border-ink-100 rounded-lg px-3 py-2.5">
            <div class="text-[10px] uppercase font-semibold text-ink-400 tracking-wide">{{ __('A la entrega') }}</div>
        ...
        <div class="sm:col-span-2 lg:col-span-3 border border-ink-100 rounded-lg px-3 py-2.5 flex items-center justify-between">
            ...
            @if(! empty($r->budget_notes))
                <div class="max-w-[60%] text-[12px] text-ink-700">
                    <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400">{{ __('Notas de tu asesor') }}</div>
    ...
    @if($isSent && ! $accepted)
        <div class="px-5 py-4 border-t border-ink-100 bg-white space-y-3">
            <div class="flex flex-wrap items-center gap-2 justify-between">
                <div class="text-[12px] text-ink-700">{{ __('¿Estás conforme con este plan?') }}</div>
                <div class="flex items-center gap-2">
                    <button type="button" onclick="document.getElementById('plan-obs-form').classList.toggle('hidden')" class="cli-btn cli-btn-ghost text-[12px]"><i class="pi pi-comments text-[10px]"></i> {{ __('Enviar observación') }}</button>
                    <form method="POST" action="{{ route('dashboard.budget.accept', $r) }}" class="m-0" onsubmit="event.preventDefault(); acceptPlan(this);">
                        @csrf
                        <button type="submit" class="cli-btn cli-btn-primary text-[12px]"><i class="pi pi-check text-[10px]"></i> {{ __('Marcar como conforme') }}</button>
                    </form>
                </div>
            </div>
            ...
            <form id="plan-obs-form" method="POST" action="{{ route('dashboard.budget.observation', $r) }}" class="hidden space-y-2 m-0">
                @csrf
                <textarea name="message" rows="3" required maxlength="2000" placeholder="{{ __('Contale a tu asesor qué te gustaría ajustar (ej. más cuotas, menor inicial, etc.)…') }}" class="cli-input w-full pl-3 pt-2 h-auto resize-none"></textarea>
                <div class="flex items-center gap-2 justify-end">
                    <button type="button" onclick="document.getElementById('plan-obs-form').classList.add('hidden')" class="cli-btn cli-btn-ghost text-[12px]">{{ __('Cancelar') }}</button>
