...
<div class="p-6 sm:p-7 max-w-4xl">
    ...
    <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="crm-card p-5 sm:p-6">
            ...
            <div class="text-[12px] text-ink-500 mb-4">{{ __('Se mostrará en el CRM y en las conversaciones con clientes.') }}</div>
        ...
        <div class="crm-card p-5 sm:p-6">
            ...
            <div class="text-[12px] text-ink-500 mb-4">{{ __('Estos datos identifican al administrador en el sistema.') }}</div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="text-[12px] font-semibold text-ink-700">{{ __('Nombre') }}</label>
                    <input type="text" name="first_name" value="{{ old('first_name', $user->first_name) }}" class="crm-input pl-3 mt-1" placeholder="{{ __('Nombre') }}">
                </div>
                <div>
                    <label class="text-[12px] font-semibold text-ink-700">{{ __('Apellido') }}</label>
                    <input type="text" name="last_name" value="{{ old('last_name', $user->last_name) }}" class="crm-input pl-3 mt-1" placeholder="{{ __('Apellido') }}">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="country" value="{{ old('country', $user->country) }}" class="crm-input pl-3 mt-1" placeholder="{{ __('Colombia') }}">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.crm.dashboard') }}" class="crm-btn crm-btn-ghost">{{ __('Cancelar') }}</a>
