...
<div class="p-4 sm:p-6 lg:p-8 space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <button type="button" onclick="document.getElementById('modal-nueva-aprob').showModal()" class="crm-btn crm-btn-primary"><i class="pi pi-plus"></i> Nueva aprobación</button>
        </div>
    </div>
        ...
        <div class="divide-y divide-ink-100">
            @foreach($pendingUsersList as $u)
                ...
                <div class="px-5 py-4 flex items-center gap-4">
                    ...
                    <span class="crm-pill bg-info-soft text-info">NUEVO USUARIO</span>
                    ...
                    <form method="POST" action="{{ route('admin.users.verify-kyc', $u->id) }}" class="flex items-center gap-1 m-0">@csrf
                        <button type="submit" name="decision" value="approved" class="crm-btn crm-btn-primary text-[11px] py-1 px-3">Aprobar</button>
                        <button type="submit" name="decision" value="rejected" class="crm-btn crm-btn-ghost text-[11px] py-1 px-3">Rechazar</button>
                    </form>
                </div>
            @endforeach
            ...
            @foreach($pendingKycList as $k)
                ...
                <div class="px-5 py-4 flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-1">
                        @if($r)
                            <a href="{{ route('admin.crm.expediente.detalle', $r->id) }}" class="crm-btn crm-btn-ghost text-[11px] py-1 px-3" title="Ver expediente"><i class="pi pi-folder text-[10px]"></i> Expediente</a>
                        @endif
                        <button type="button" onclick="document.getElementById('modal-kyc-aprob-{{ $k->id }}').showModal()" class="crm-btn crm-btn-ghost text-[11px] py-1 px-3" title="Ver detalles del KYC"><i class="pi pi-eye text-[10px]"></i> Ver</button>
                        <form method="POST" action="{{ route('documents.approve', $k->id) }}" class="m-0">@csrf
                            <button type="submit" class="crm-btn crm-btn-primary text-[11px] py-1 px-3" title="Aprobar KYC"><i class="pi pi-check text-[10px]"></i> Aprobar</button>
                        </form>
                    </div>
                </div>
            @endforeach
                ...
                <div class="px-5 py-4 flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <form method="POST" action="{{ route('admin.payments.approve', $p->id) }}" class="m-0">@csrf
                            <button type="submit" name="decision" value="approved" class="crm-btn crm-btn-primary text-[11px] py-1 px-3" title="Aprobar pago"><i class="pi pi-check text-[10px]"></i> Aprobar</button>
                            <button type="submit" name="decision" value="rejected" class="crm-btn crm-btn-ghost text-[11px] py-1 px-3" title="Rechazar pago"><i class="pi pi-times text-[10px]"></i> Rechazar</button>
                        </form>
                    </div>
                </div>
            @endforeach
                ...
                <div class="px-5 py-4 flex items-center gap-4">
                    ...
                    @if($a->status === 'pendiente')
                        <form method="POST" action="{{ route('admin.crm.aprobaciones.decide', $a->id) }}" class="flex items-center gap-1 m-0">@csrf
                            <button type="submit" name="decision" value="aprobada" class="crm-btn crm-btn-primary text-[11px] py-1 px-3">Aprobar</button>
                            <button type="submit" name="decision" value="rechazada" class="crm-btn crm-btn-ghost text-[11px] py-1 px-3">Rechazar</button>
                        </form>
                    @endif
                </div>
            @empty
                @if($pendingUsersList->isEmpty() && $pendingKycList->isEmpty())
                    <div class="px-5 py-8 text-center text-[12px] text-ink-500">No hay aprobaciones registradas.</div>
                @endif
            @endforelse
        </div>
    </div>
...
<dialog id="modal-nueva-aprob" class="rounded-2xl p-0 backdrop:bg-black/40 m-auto">
    <form method="POST" action="{{ route('admin.crm.aprobaciones.store') }}" class="w-[520px] bg-white rounded-2xl overflow-hidden">@csrf
        <div class="px-6 py-4 border-b border-ink-100 text-[15px] font-bold text-ink-900">Nueva aprobación</div>
        <div class="p-6 space-y-3">
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="text-[12px] font-semibold text-ink-700">Prioridad</label>
            ...
            <div><label class="text-[12px] font-semibold text-ink-700">Solicitante</label><input type="text" name="requested_by" required class="crm-input pl-3 mt-1"></div>
            <div><label class="text-[12px] font-semibold text-ink-700">Monto/Condición</label><input type="text" name="amount_or_condition" class="crm-input pl-3 mt-1" placeholder="$8,000 / 8% descuento"></div>
            <div>
                <label class="text-[12px] font-semibold text-ink-700">Expediente vinculado</label>
        ...
        <div class="px-6 py-4 border-t border-ink-100 flex items-center gap-2 justify-end bg-ink-50">
            ...
            <button type="submit" class="crm-btn crm-btn-primary">Crear aprobación</button>
        </div>
    </form>
</dialog>
