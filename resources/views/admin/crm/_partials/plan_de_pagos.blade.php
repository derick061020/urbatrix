...
<div class="crm-card overflow-hidden">
    <div class="px-4 py-3 bg-ink-50 border-b border-ink-100 flex items-center gap-3">
        ...
        <div class="text-[13px] font-bold text-ink-700">{{ __('Plan de pagos') }}</div>
                    ...
                    <div class="flex {{ $fromAdmin ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[80%] rounded-xl px-3 py-2 text-[12px] {{ $isAcceptEntry ? 'bg-ok-soft border border-ok/30 text-ok-dark' : ($fromAdmin ? 'bg-brand text-white' : 'bg-white border border-ink-200 text-ink-800') }}">
                            <div class="text-[10px] uppercase tracking-wide opacity-70 mb-1 flex items-center gap-1">
                                ...
                                @if($isAcceptEntry)<span class="font-bold ml-1">{{ __('Aceptación') }}</span>@endif
                            </div>
                            <div>{{ $o['message'] ?? '' }}</div>
                        </div>
                    </div>
        ...
        <div class="p-5">
            <div class="rounded-lg border border-ok/30 bg-ok-soft/40 px-4 py-3 flex items-center gap-3">
                ...
                <div class="text-[12px] text-ok-dark">
                    <div class="font-semibold">{{ __('Plan de pagos firmado por el cliente') }}</div>
                    <div class="opacity-80">{{ __('El presupuesto quedó cerrado. La pestaña "Plan de Pagos" ya está habilitada.') }}</div>
                </div>
            </div>
        </div>
    @else
    <form method="POST" action="{{ route('admin.crm.budget.save', $r->id) }}" class="p-5 space-y-4 m-0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label class="text-[12px] font-semibold text-ink-700">{{ __('% Construcción') }}</label>
                <div class="relative mt-1">
            ...
            <div>
                <label class="text-[12px] font-semibold text-ink-700">{{ __('Cuotas (construcción)') }}</label>
        ...
        <div>
            <label class="text-[12px] font-semibold text-ink-700">{{ __('Notas (visible al cliente)') }}</label>
            <textarea name="budget_notes" rows="2" {{ $isLocked ? 'disabled' : '' }} class="crm-input pl-3 pt-2 mt-1 h-auto resize-none" placeholder="{{ __('Detalles, vencimientos, condiciones especiales…') }}">{{ $r->budget_notes }}</textarea>
        </div>

        @if(! empty($observations) && ! $isLocked)
            <div>
                <label class="text-[12px] font-semibold text-ink-700">{{ __('Respuesta al cliente') }}</label>
                <textarea name="admin_reply" rows="2" class="crm-input pl-3 pt-2 mt-1 h-auto resize-none" placeholder="{{ __('Tu respuesta a la última observación del cliente (opcional)…') }}"></textarea>
            </div>
        @endif
