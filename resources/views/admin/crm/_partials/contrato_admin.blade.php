    ...
    @if(! $signed)
        <div class="p-5 space-y-3">
            <form method="POST" action="{{ route('admin.crm.contract.upload', $d->id) }}" enctype="multipart/form-data" class="space-y-3 m-0">
                @csrf
                <div>
                    <label class="text-[12px] font-semibold text-ink-700">Subir versión modificada del contrato</label>
                ...
                <div>
                    <label class="text-[12px] font-semibold text-ink-700">Mensaje al cliente (opcional)</label>
                    <textarea name="admin_reply" rows="2" class="crm-input pl-3 pt-2 mt-1 h-auto resize-none" placeholder="Explicá qué cambios incluye esta versión…"></textarea>
                </div>
                <div class="flex justify-end">
                    <button type="submit" class="crm-btn crm-btn-primary"><i class="pi pi-upload"></i> Enviar versión modificada</button>
                </div>
            </form>
            ...
            @if(! empty($obs))
                ...
                <form method="POST" action="{{ route('admin.crm.contract.reply', $d->id) }}" class="space-y-2 m-0">
                    @csrf
                    <label class="text-[12px] font-semibold text-ink-700">Responder sin subir archivo nuevo</label>
                    <textarea name="admin_reply" required rows="2" class="crm-input pl-3 pt-2 mt-1 h-auto resize-none" placeholder="Responder a la última observación…"></textarea>
                    <div class="flex justify-end">
                        <button type="submit" class="crm-btn crm-btn-ghost"><i class="pi pi-send"></i> Enviar respuesta</button>
                    </div>
                </form>
            @endif
        </div>
