...
<dialog id="modal-solicitar-documento" class="rounded-2xl p-0 backdrop:bg-black/40 m-auto">
    <form method="POST" action="{{ route('admin.crm.document.request', $reservation->id) }}" class="w-[520px] bg-white rounded-2xl overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-ink-100 flex items-center gap-3">
            ...
            <div class="text-[15px] font-bold text-ink-900 flex-1">Solicitar documento al cliente</div>
        ...
        <div class="p-6 space-y-4">
            <p class="text-[12px] text-ink-500">El cliente verá este documento como requerido en su panel y podrá subir el archivo desde ahí.</p>
            <div>
                <label class="text-[12px] font-semibold text-ink-700">Nombre del documento</label>
                <input type="text" name="title" required placeholder="Comprobante de ingresos" class="crm-input pl-3 mt-1">
            </div>
            <div>
                <label class="text-[12px] font-semibold text-ink-700">Descripción / instrucciones <span class="text-ink-400 font-normal">(opcional)</span></label>
                <input type="text" name="description" placeholder="Últimos 3 estados de cuenta o carta laboral" class="crm-input pl-3 mt-1">
            </div>
            <div>
                <label class="text-[12px] font-semibold text-ink-700">Fecha límite <span class="text-ink-400 font-normal">(opcional)</span></label>
        ...
        <div class="px-6 py-4 border-t border-ink-100 flex items-center gap-2 justify-end bg-ink-50">
            ...
            <button type="submit" class="crm-btn crm-btn-primary"><i class="pi pi-send"></i> Solicitar al cliente</button>
        </div>
    </form>
</dialog>
