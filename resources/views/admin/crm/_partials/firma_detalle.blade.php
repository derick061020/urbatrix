...
<div class="crm-card overflow-hidden">
    ...
    <div class="p-4 grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4">
        {{-- Signature image --}}
        <div>
            <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400 mb-1.5">{{ __('Firma manuscrita') }}</div>
            <div class="rounded-xl border border-ink-200 bg-white h-[88px] flex items-center justify-center overflow-hidden p-2">
                @if($sigImg)
                    <img src="{{ $sigImg }}" alt="{{ __('Firma del cliente') }}" class="max-h-full max-w-full object-contain">
                @else
                    <span class="text-[11px] text-ink-400">{{ __('Sin imagen de firma') }}</span>
                @endif
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
            <div>
                <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400">{{ __('Firmante') }}</div>
            ...
            <div>
                <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400">{{ __('Dirección IP') }}</div>
                <div class="text-[13px] font-semibold text-ink-900 mt-0.5 font-mono">{{ $sigIp ?: '—' }}</div>
            </div>
            <div>
                <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400">{{ __('Términos aceptados') }}</div>
            ...
            <div class="sm:col-span-2">
                <div class="text-[10px] uppercase tracking-wide font-semibold text-ink-400">{{ __('Dispositivo / navegador') }}</div>
