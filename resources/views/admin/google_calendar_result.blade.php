...
<body>
    @if($ok)
        <h1 class="ok">{{ __('✓ Listo. Tu refresh token de Google Calendar:') }}</h1>
        <code class="token">{{ $token }}</code>
        <h3>{{ __('Pasos finales:') }}</h3>
        <ol>
        ...
        <details>
            <summary>{{ __('Respuesta completa de Google') }}</summary>
            <pre>{{ $body }}</pre>
        </details>
    @else
        <h1 class="err">{{ __('✗ No se pudo obtener el refresh token.') }}</h1>
