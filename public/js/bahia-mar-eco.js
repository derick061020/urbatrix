...
(() => {
  ...
  /* ⚠️ ESTA FUNCIÓN DECIDE TAMBIÉN EL COLOR DE LA TARJETA, aunque no lo
     parezca: `data-eco-shade` es lo que el CSS mira para pintarla en salvia
     (12.1). Las que se quedan sin hoja se quedan en blanco. Son la misma
     decisión, así que no hay un segundo reparto que mantener en sincronía. */
  function pintarSombrasCatalogo() {
    document.querySelectorAll('.fg-units-grid > .fg-card').forEach((card, i) => {
      ...
      /* Al filtrar, una tarjeta puede caer en otro índice: la hoja que ya no
         le toca se retira en vez de quedarse pegada — y con ella se va el
         verde de la superficie, que cuelga del mismo atributo. */
      if (!especie) {
